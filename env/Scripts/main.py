...
def understand_image(base64_image, api_key):
    prompt = "Analyze the content of this image and write a creative, engaging story that bring the scene to life. Describe the characters, settings \
        and actions in a way that would captivate a young audience:"
    inference_params = dict(temperature=.2, image_base64=base64_image, api_key=api_key)
    model_prediction = Model("https://clarifai.com/openai/chat-completion/models/gpt-4-vision"
                       ).predict_by_bytes(prompt.encode(), input_type="text", inference_params=inference_params)
...
def text_to_speech(input_text, api_key):
    ...
    model_prediction = Model("https://clarifai.com/openai/tts/models/openai-tts-1"
                            ).predict_by_bytes(input_text.encode(), input_type="text", inference_params=inference_params)
...
def main():
    ...
    with st.sidebar:
        st.header("Controls")
        image_description = st.text_area("Describe for image generation", height=100)
